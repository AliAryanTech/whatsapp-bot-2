...
module.exports = {
    ...
    exec: async(ZimBotInc, m, { text }) => {
        ...
        ZimBotInc.sendFile(m.from, fetch.result.media[1].url || fetch.result.media[0].url, "", m, { jpegThumbnail: await fetchBuffer(fetch.result.thumbnail), caption: `*💮 Title :* ${fetch.result.tile}\n*💮 Source Url :* ${fetch.result.url}` })
    },
    isQuery: true
}