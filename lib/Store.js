...
function bind(ZimBotInc) {
    ...
    ZimBotInc.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    ZimBotInc.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    ZimBotInc.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   ZimBotInc.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    ZimBotInc.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    ZimBotInc.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    ZimBotInc.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    ZimBotInc.ev.on('chats.update', updates => {
    ...
    ZimBotInc.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    ZimBotInc.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
                    ...
                    if (type === 'notify') {
                        if (!chats.get(jid)) {
                            ZimBotInc.ev.emit('chats.upsert', [
                                {
                                    id: jid,
    ...
    /**ZimBotInc.ev.on('messages.update', updates => {
    ...
    ZimBotInc.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    ZimBotInc.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    ZimBotInc.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (ZimBotInc === null || ZimBotInc === void 0 ? void 0 : ZimBotInc.messageInfo(remoteJid, id))
                if (msg) {
